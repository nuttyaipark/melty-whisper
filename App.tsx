...
// Positioning data for floating bubbles (Percentage values)
...
const App: React.FC = () => {
  ...
  const [interactionBuffer, setInteractionBuffer] = useState<AudioBuffer | null>(null);
  ...
  useEffect(() => {
    const initAudio = async () => {
      try {
        ...
        // Fix for GitHub Pages sub-path
        const basePath = import.meta.env.BASE_URL === '/' ? '' : import.meta.env.BASE_URL;
        const soundPath = `${basePath}sounds/bubble.wav`.replace('//', '/');
        const buffer = await loadAudio(soundPath, ctx);
        setInteractionBuffer(buffer);
      } catch (e) {
        console.warn("Failed to load bubble noise:", e);
      }
    };
    initAudio();
  }, []);
  ...
  const handleNextPhase = (next: AppPhase) => {
    playInteractionSound(getAudioContext(), interactionBuffer);
    setPhase(next);
  };
            ...
            {moodKeys.map((moodId, index) => {
              ...
              return (
                <FloatingBubble
                  ...
                  className={`cursor-pointer transition-all duration-500 pointer-events-auto ${isSelected ? 'z-30 scale-125' : 'z-10 hover:z-20 hover:scale-110 opacity-70 hover:opacity-100'
                    }`}
                >
                  <div className={`
